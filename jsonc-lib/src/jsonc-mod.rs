...
pub mod jsonc_mod {
    ...
    use super::cglue; // restrict jsonc C-binding visible only internally
            ...
            unsafe {
                ...
                let cbuffer = if format.alternate() {
                    ...
                    cglue::json_object_to_json_string_ext(
                        jso,
                        cglue::JSON_C_TO_STRING_PRETTY as i32,
                    )
                } else {
                    // {}
                    cglue::json_object_to_json_string_ext(
                        jso,
                        (cglue::JSON_C_TO_STRING_NOSLASHESCAPE | cglue::JSON_C_TO_STRING_NOZERO)
                            as i32,
                    )
                };
    ...
    impl JsoncObj {
        ...
        {
            unsafe {
                let result = if cglue::json_object_is_type(
                    self.jso,
                    cglue::json_type_json_type_array,
                ) == 0
                {
        ...
        pub fn expand(&self) -> Vec<Jentry> {
            ...
            while entry != 0 as *mut cglue::lh_entry {
                let key= unsafe {CStr::from_ptr((*entry).k as *mut i8).to_owned().to_str().unwrap().to_owned()};
        ...
        pub fn equal(&mut self, jsonc: JsoncObj) -> Result<(),&'static str> {
            if unsafe {cglue::json_object_get_type(self.jso)} != unsafe {cglue::json_object_get_type(jsonc.jso)} {
                Err("jtype diverge")
